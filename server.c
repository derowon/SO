...
void answer(Package * data){
  printf("ESTE ES EL LEGAJO : %d \n", data->data.sign.studentID);
  printf("ESTE ES EL CODIGO DE MATERIA: %d \n", data->data.subC);
...
Package * subscribeSubject_db(Package * data){
  ...
  buffer = subscribeSubject(data->data.subC,data->data.sign.studentID);
...
Package * cancelSubscription_db(Package * data){
  ...
  buffer = cancelSubscription( data->data.subC, data->data.sign.studentID);
...
Package * cupo_db(Package * data){
  char *buffer;
  printf("Cupo de la materia con codigo %d \n",data->data.subC);

  buffer = getCupo(data->data.subC);
  memcpy(data->data.response, buffer,strlen(buffer));
...
Package * misMaterias_db(Package * data){
  char* buffer = getSubjectsByStudentCode(data->data.sign.studentID);
  memcpy(data->data.response, buffer,strlen(buffer));
